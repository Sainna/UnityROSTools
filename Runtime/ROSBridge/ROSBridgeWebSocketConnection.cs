 ...
 namespace ROSBridgeLib {
    ...
    /// </remarks>
 	public class ROSBridgeWebSocketConnection {
        ...
        public WebSocketState GetWSState()
        {
            return _ws.State;
        }

        public void OnSocketOpen(WebSocketOpenEventHandler onOpenCallback)
        {
        ...
        public void AddSubscriber(string topicName, ROSTopicSubscriber subscriber)
        {
            ...
            if (_connected)
            {
                _ws.SendText(ROSBridgeMsg.Subscribe(topicName, subscriber.GetMessageType(topicName)));
        ...
        public void RemoveSubscriber(string topicName)
        {
            if (_connected)
            {
                ...
                {
                    _ws.SendText(ROSBridgeMsg.UnSubscribe(topicName));
        ...
        public void AddPublisher(string topicName, string message_type)
        {
            ...
            if (_connected)
            {
                _ws.SendText(ROSBridgeMsg.Advertise(topicName, message_type));
        ...
        public void Disconnect() {
            ...
            foreach(string topicName in subscribers.Keys)
            {
		 	...
		 	foreach(string topicName in static_publishers.Keys) {
		 		_ws.SendText(ROSBridgeMsg.UnAdvertise (topicName));
		...
		private void Run() {
		 	...
		 	_ws.OnMessage += (e) => this.OnMessage(System.Text.Encoding.UTF8.GetString(e));
		 	_ws.Connect();
            _connected = true;

		 	foreach(KeyValuePair<string, List<Type>> topic in static_subscribers) {
		 		_ws.SendText(ROSBridgeMsg.Subscribe (topic.Key, GetMessageType (topic.Value[0])));
            ...
            foreach(KeyValuePair<string, List<ROSTopicSubscriber>> topic in subscribers)
            {
                ...
                {
                    _ws.SendText(ROSBridgeMsg.Subscribe(topic.Key, topic.Value[0].GetMessageType(topic.Key)));
		 	...
		 	foreach(KeyValuePair<string, List<Type>> topic in static_publishers) {
		 		_ws.SendText(ROSBridgeMsg.Advertise (topic.Key, GetMessageType(topic.Value[0])));
            ...
            foreach(KeyValuePair<string, string> topic in publishTopic_to_messageType)
            {
                ...
                {
                    _ws.SendText(ROSBridgeMsg.Advertise(topic.Key, topic.Value));
        ...
        public void AddSubscriber(Type subscriber)
        {
            ...
            if (_connected)
            {
                _ws.SendText(ROSBridgeMsg.Subscribe(topicName, GetMessageType(subscriber)));
        ...
        public void AddPublisher(Type publisher)
        {
            ...
            if (_connected)
            {
                _ws.SendText(ROSBridgeMsg.Advertise(topicName, GetMessageType(publisher)));
